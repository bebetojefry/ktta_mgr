...
use App\FrontBundle\Form\ApproveType;
...
class PlayerController extends Controller {
    ...
    public function approveAction(Request $request, Player $player) {
        $dm = $this->getDoctrine()->getManager();
        $form = $this->createForm(new ApproveType(), $player);
        $code = FormHelper::FORM;
        if($request->isMethod('POST')){
            $form->handleRequest($request);
            if($form->isValid()){
                $player = $form->getData();
                $player->setStatus(Player::APPROVED);
                $dm->persist($player);
                $dm->flush();
                $this->get('session')->getFlashBag()->add('success', 'Player successfully approved');
                $code = FormHelper::REFRESH;
            } else {
                $code = FormHelper::REFRESH_FORM;
            }
        ...
        $body = $this->renderView('AppFrontBundle:Player:approve_form.html.twig',
            array('form' => $form->createView())
        );
